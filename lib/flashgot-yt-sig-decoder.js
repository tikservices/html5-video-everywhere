...
var fg = {
    log: console.log.bind(console, "[FLASHGOT]"),
    getPref: (title, deflt) => service.get(title, deflt),
    setPref: (title, val) => service.set(title, val)
};

var Youtube = {

    fix_signature: function(data, streams) {
        ...
        streams.forEach(function(s) {
            try {
                ...
                if (sig) {
                    s.url += "&signature=" + encodeURIComponent(sig);
                }
            } catch (x) {
