...
Youtube.fix_signature = function(data, fmts, w, cb) {
  try {
    this.create_sig_decoder(w, (signature_decoder) => {
      for (let itag in fmts) {
        if (fmts[itag].url.search("signature=") > 0)
          continue;
        console.log("fixing stream", itag);
        try {
          ...
          if (sig) {
            console.log("Fmt", itag, "url fixed:", sig);
            fmts[itag].url += "&signature=" + encodeURIComponent(sig);
          } else {
            console.log("Failed to fix fmt", itag, "signature");
          }
        } catch (x) {
          console.log("Error calling YouTube.decode_signature_func: " + (x.message || x) + "\n" + x.stack);
        }
      }
      try {
    ...
    console.log("Error creating signature decoder: " + (x.message || x) + "\n" + x.stack);
    cb({});
  }
};
Youtube.create_sig_decoder = function (w, cb) {
