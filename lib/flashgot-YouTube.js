...
Youtube.fix_signature = function(data, fmts, cb, w) {
  try {
    this.create_sig_decoder((signature_decoder) => {
      for (let itag in fmts) {
        if (fmts[itag].url.search("signature=") > 0)
          continue;
        console.log("[CORE] fixing stream", itag);
        try {
          ...
          if (sig) {
            console.log("[CORE] Fmt", itag, "url fixed:", sig);
            fmts[itag].url += "&signature=" + encodeURIComponent(sig);
          } else {
            console.log("[CORE] Failed to fix fmt", itag, "signature");
          }
        } catch (x) {
          console.log("[CORE] Error calling YouTube.decode_signature_func: " + (x.message || x) + "\n" + x.stack);
        }
      }
      try {
    ...
    console.log("[CORE] Error creating signature decoder: " + (x.message || x) + "\n" + x.stack);
    cb({});
  }
};
Youtube.create_sig_decoder = function (cb, w) {
  // Wrapper around create_sig_decoder with callback and
  // refresh_signature_decoder
