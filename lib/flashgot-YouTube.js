...
var fg = {
    ...
    getPref: (title, deflt) => service.get("flashgot." + title, deflt),
    setPref: (title, val) => service.set("flashgot." + title, val)
};

var Youtube = {

    fix_signature: function(data, fmts, cb) {
        try {
            this.create_signature_decoder((signature_decoder) => {
    ...
    sniff: function(w, callback /*= null*/ , force /*= false*/ ) {
        ...
        var stream_listener = {
            ...
            onStartRequest: function(req /*, ctx*/ ) {
                ...
                if (!((channel instanceof Ci.nsIHttpChannel) && components.isSuccessCode(channel.status) && channel.responseStatus === 200)) {
...
SDAWorker.prototype = {
    // public
    start: function() {
        var worker = this.worker = new Worker("flashgot-YoutubeSwf.js");
    ...
    finish: function() {
        ...
        try {
            ...
            this.worker["SDAWorker::this"] = this.worker.onmessage = this.worker.onerror = null;
            this.worker = null;
        } catch (x) {
