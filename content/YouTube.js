...
class YouTube extends Module {
        ...
        .then((conf) => {
          try {
            ...
            this.vp.props({
              id : "video_player",
              className : conf.className || "",
              autoplay : autoPlay(location.search.search("autoplay=") === -1
                                      ? !conf.isEmbed
  ...
  errorMessage(msg, conf) {
    ...
    if (conf && conf.isWatch)
      this.vp.containerProps({className : " player-height player-width player-api"});
    if (conf && conf.isChannel)
      this.vp.containerProps({className : " c4-player-container"}); //" html5-main-video";
    if (conf && conf.isEmbed) {
      this.vp.containerProps({className : " full-frame"});
    }
    this.vp.containerStyle({
      background :
          "linear-gradient(to bottom, #383838 0px, #131313 100%) repeat scroll 0% 0% #262626"
    });
    this.vp.error(msg);
  }

  getPlayerContainer(conf) {
    if (conf.isWatch) return document.getElementById("player-mole-container");
  ...
  getConfig() {
    return new Promise((resolve, reject) => {
      ...
      if (!conf.id)
        reject({error : "PLAYER_ID_NOT_FOUND", conf : conf});
      else
        resolve(conf);
    });
  }
  ...
  getVideoInfo(conf) {
    return new Promise((resolve, reject) => {
             ...
             if (typeof unsafeWindow !== "undefined" && unsafeWindow.ytplayer &&
                 unsafeWindow.ytplayer.config) {
               ...
               if (unsafeWindow.ytplayer.config.args.caption_tracks)
                 conf.tracks = this.parse(unsafeWindow.ytplayer.config.args.caption_tracks, true);
               this.swf_url  = unsafeWindow.ytplayer.config.url;
               resolve(conf);
             } else {
               asyncGet(INFO_URL + conf.id, {}, "text/plain").then((data) => {
                 ...
                 if (data.status === "fail") {
                   return reject({error : "VIDEO_URL_UNACCESSIBLE", data : data, conf : conf});
                   }
                 // get the poster url
  ...
  fixSignature(conf) {
    return new Promise((resolve, reject) => {
      self.port.emit("fix_signature", {fmts : conf.fmts, swf_url : this.swf_url});
