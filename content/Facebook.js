...
class Facebook extends Module {
  ...
  onInteractive() {
    ...
    vp.srcs({"medium/mp4" : params.video_data[0].sd_src, "high/mp4" : params.video_data[0].hd_src});
    vp.props({controls : true, autoplay : autoPlay(true), preload : preLoad(), loop : isLoop()});
    vp.style({width : "100%", heigth : "100%"});
    vp.setup();
  }
}
